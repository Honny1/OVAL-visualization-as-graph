...
def test_prepare_data_not_implemented_error():
    src = 'test_data/ssg-fedora-ds-arf.xml'
    part_of_id_rule = 'xccdf_org.ssgproject.'
    client = get_client(src, part_of_id_rule)
    with pytest.raises(NotImplementedError):
        assert client.prepare_data(['rule-id'])
...
def test_get_lines_of_wanted_not_selected_rules_not_implemented_error():
    src = 'test_data/ssg-fedora-ds-arf.xml'
    part_of_id_rule = 'xccdf_org.ssgproject.'
    client = get_client(src, part_of_id_rule)
    with pytest.raises(NotImplementedError):
        assert client._get_lines_of_wanted_not_selected_rules()
